...
class MappingServer():
    ...
    def __init__(self):
        ...
        self.coverage=dict() # A holder which will track coverage of each sequence in a reference
    ...
    def databasemonitor(self):
        while True:
            ...
            for reference in self.mappingobjects:
                print (reference)
                if self.mappingobjects[reference]["last_used"] < time.time()-self.interval:
    ...
    def target_coverage(self):
        ...
        for refname in self.coverage:
            if self.coverage[refname]["length"]>0:
                if self.coverage[refname]["bases"]/self.coverage[refname]["length"] >= self.cov_target:
                    coverage_results[refname]=self.coverage[refname]["bases"] / self.coverage[refname][ "length"]
        return coverage_results
    ...
    def load_index(self,reference,filepath):
        ...
        self.mappingobjects[reference]["reference"]=mp.Aligner(filepath, preset='map-ont', )
        self.mappingobjects[reference]["last_used"]=time.time()
        for refname in self.mappingobjects[reference]["reference"].seq_names:
            self.add_ref_coverage(refname,len(self.mappingobjects[reference]["reference"].seq(refname)))
    ...
    def map_sequence(self,reference,sequence,trackcov=True):
        ...
        results = list()
        for hit in self.mappingobjects[reference]["reference"].map(sequence["sequence"]):
